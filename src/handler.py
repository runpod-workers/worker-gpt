...
if __name__ == "__main__":
    ...
    if args.model_name == "gpt-neo-1.3B":
        model = GPTNeoForCausalLM.from_pretrained(
            "EleutherAI/gpt-neo-1.3B", local_files_only=True
        ).to(device)
        ...
        model = GPTNeoForCausalLM.from_pretrained(
            "EleutherAI/gpt-neo-2.7B", local_files_only=True, torch_dtype=torch.float16
        ).to(device)
        tokenizer = GPT2Tokenizer.from_pretrained(
            "EleutherAI/gpt-neo-2.7B", local_files_only=True
        )
